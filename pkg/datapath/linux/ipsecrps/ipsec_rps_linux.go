...
func (u userFlags) Flags(flags *pflag.FlagSet) {
	flags.Bool("enable-ipsec-acceleration", u.EnableIpsecAcceleration, "Enable acceleration for IPSec using RPS.")
}
...
func newConfig(in newConfigIn) (Config, error) {
	...
	if in.XDPCfg.Disabled() || in.XDPCfg.AccelerationMode() != xdp.AccelerationModeNative {
		return Config{}, fmt.Errorf("ipsec rps can only be enabled if XDP is enabled in native mode")
	}
	// Is the CPUMap enabled?
	if !in.CPUMapCfg.Enabled() {
		return Config{}, fmt.Errorf("ipsec rps can only be enabled if the cpu map is enabled")
	}
...
func (cfg Config) datapathConfigProvider() dpcfgdef.NodeOut {
	...
	if cfg.Enabled() {
		...
		defines["IPSEC_RPS_NCPU"] = strconv.FormatInt(int64(runtime.NumCPU()), 10)
	}
